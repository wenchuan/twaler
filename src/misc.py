...
class Logger:
  def __init__(self,name,dir_log):
    ...
    if not os.path.exists(dir_log):
      os.makedirs(dir_log)
    self.errorlog = open(os.path.join(dir_log, self.name+".%s.error.log" % timefunctions.datestamp()), "a")

  def log(self,errmsg):
    msg = "[" + self.name + "] %s: %s" % (timefunctions.datestamp(), errmsg)
    self.errorlog.write(msg + "\n")

  def verbose_log(self,errmsg):
    msg = "[" + self.name + "] %s: %s" % (timefunctions.datestamp(), errmsg)
    print(msg)
    self.errorlog.write(msg + "\n")
...
class timefunctions:
  ...
  #Note: The time used here is GMT time!!
  #unfortunately, not all platforms support '%z' in strptime, thus we need to manually parse it
...
class CacheAccessor():
  ...
  def get_crawl_dir(self, basepath, uid, create=False, listname=None):
    ...
    d1 = int(nid / 10) % 10
    d2 = int(nid / 100) % 10
    cache_path = os.path.join(basepath, str(d0))
    cache_path = os.path.join(cache_path, str(d1))
    cache_path = os.path.join(cache_path, str(d2))
    cache_path = os.path.join(cache_path, uid)
    if (listname):
      cache_path = os.path.join(cache_path, "lists")
      cache_path = os.path.join(cache_path, listname)
  ...
  def get_cache_dir(self, uid, create=False, listname=None):
    """Return, and if needed create, the path associated with this user id."""
    return self.get_crawl_dir(self.cache_dir, uid, create=create, listname=listname)

  def check_cache(self, uid):
    """return last read tweet id if cache includes it
       otherwise return -1"""
...
class mysql_db():
  ...
  def __init__(self,host,user,password,db,log):
    try:
        self.conn = mysql.connector.connect(host=host,user=user,password=password, db=db)
